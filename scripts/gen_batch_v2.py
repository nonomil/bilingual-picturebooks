...
ACCOUNTS = [
    ...
    {"email": "UribeTocayo@gmail.com",     "pass": "ivhuf19nskw",   "totp": "37qvk25ebaefheyyqwei2nqvqdcylge5"},
]

MAX_PER_ACCOUNT = 15       # Switch account after N images
SLEEP_BETWEEN = 40         # Seconds between images (30-60 recommended)
RETRY_DELAY = 70           # Wait before retrying a failed image
...
def send_and_generate(page, prompt):
    page.evaluate(f'''()=>{{const ta=document.querySelector("textarea");if(!ta)return;ta.focus();ta.value="{prompt}";ta.dispatchEvent(new Event("input",{{bubbles:true}}));ta.dispatchEvent(new Event("change",{{bubbles:true}}))}}''')
    time.sleep(1)
    before = count_imgs(page)
    page.evaluate('()=>{for(const b of Array.from(document.querySelectorAll("button"))){if(b.textContent.includes("Run")&&b.offsetParent!==null){b.click();return}}}')
    start = time.time()
...
def main():
    ...
    with Camoufox(
        ...
        if not enter_chat(page):
            print("ERROR: Could not enter chat interface")
            return
        ...
        for i, (ch, fname, prompt, key) in enumerate(work):
            ...
            print(f"[{i+1}/{len(work)}] ch{ch}/{fname} (acct: {acct['email'][:8]}..., retries: {fail_record})")
